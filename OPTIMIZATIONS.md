...
- **Follow-ups / dead ends:**
  - **Closed:** the bincode 2.0.1 upgrade itself, *with fixed-int config*. The improvement on codec-touching paths is real (insert_2k_strings reliably -0.33% to -0.60% across all 6 runs) but too small to ship past the gate, and modify_10k is flat (the hot path is map mutation, not the codec).
